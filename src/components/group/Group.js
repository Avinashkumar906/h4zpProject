...
function Group(props) {
  return (
    <Container className={`p-0 ${props.theme}`} fluid>
      <Container className="py-5 overflow-hidden" fluid={props.fluid}>
        <Row className="justify-content-center pb-5">
          {props.data?.list?.map((m,index) => (
            <Col key={`Group-ID-${index}`} xl={props.data?.xl} lg={props.data?.lg} md={props.data?.md} sm={props.data?.sm} className="text-center py-2">
              <img alt="" className={props.data?.style} src={m.url}/>
            </Col> 
          ))}
        </Row>
        <Row className="text-center">
            <div className="h4 my-4">
              {props.data?.description.map((m,index)=><div key={`Group-desc-id-${index}`} className="mb-2">{m}</div>)}
            </div>
        </Row>
      </Container>
    </Container>
